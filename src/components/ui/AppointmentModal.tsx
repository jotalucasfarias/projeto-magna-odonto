...
export default function AppointmentModal({ onClose }: AppointmentModalProps) {
  ...
  const handleInputChange = (
    ...
    // Tratamento especial para o campo de telefone - permite apenas números e formatação
    if (name === "phone") {
  ...
  const validateField = (specificField?: string): boolean => {
    ...
    // Valida apenas campos tocados ou o campo específico que está sendo verificado
  ...
  const validateForm = (): boolean => {
    // Marca todos os campos relevantes como tocados com base na etapa atual
    ...
    // Valida todos os campos para a etapa atual
    ...
    if (!isValid) {
      // Mostra uma mensagem de erro mais específica com base nos erros de validação
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      // Verifica disponibilidade novamente antes de criar
      ...
      const whatsappNumber = "5569996021979"; // Número atualizado para o da Magna Odonto
